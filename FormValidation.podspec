#
# Be sure to run `pod lib lint FormValidation.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'FormValidation'
  s.version          = '1.1.0'
  s.summary          = 'Form validation is used to validate textfield.'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = 'Form validation is used to validate all textfield such as empry string, prefix trim, space trim, numeric char contains.'

  s.homepage         = 'https://github.com/rajesh-blazedream/FormValidation'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'Rajesh' => 'rajesh.l@blazedream.com' }
  s.source           = { :git => 'https://github.com/rajesh-blazedream/FormValidation.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '8.0'

  s.source_files = 'FormValidation/Classes/**/*'
  
  # s.resource_bundles = {
  #   'FormValidation' => ['FormValidation/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
